...
def run_server(open_browser: bool = True) -> None:
    init_db()
    # Dokploy route le trafic vers le port interne 8000.
    # Le nettoyage protège aussi contre une ancienne variable PORT mal saisie.
    raw_port = (os.environ.get("PORT", "8000") or "8000").strip()
    try:
        port = int(raw_port)
        if not 1 <= port <= 65535:
            raise ValueError
